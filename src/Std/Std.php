...
class Xapp_Util_Std extends stdClass
{
    ...
    public function set($name, $value = null)
    {
        ...
        {
            $n = array_shift($name);
            if(!isset($object->$n) || !is_object($object->$n))
            {
                $object->$n = new self();
            }
            $object = &$object->{$n};
        }
        $object->{array_shift($name)} = $value;
        return $this;
    }
    ...
    /**
     * removes property values on the given property name or path
     *
     * @error 16616
     * @param string $name expects the property name or path
     */
